...
const Form = ({formData, setFormData, setIsSubmitted, setAvatar}) => {
    ...
    const handleSubmit = (e)=>{
        e.preventDefault()
        if(formData.email && formData.fullName && formData.githubUserName && avatar){
            setIsSubmitted(true)
            alert("submitted")
            return
        ...
        <form  className='w-full md:w-1/2 xl:w-[32%] mx-auto' onSubmit={handleSubmit}>
            <div className='flex flex-col gap-2 mb-6'>
                ...
                <div className='  flex flex-col items-center h-32 p-4 border border-dashed rounded-md'>
                    ...
                    <input type="file" id="avatar"  className='opacity-0 cursor-pointer' name='avatar'  onChange={(e)=>setAvatar(e.target.files[0])} />
            ...
            <div className='flex flex-col gap-2 text-xl mb-6'>
                ...
                <input id="email" type="email" className='rounded-lg  border border-gray-400 h-12 p-3 bg-transparent' placeholder='' name='email' value={formData.email} onChange={handleChange}  />
            </div>
             <div className='flex flex-col gap-2 text-xl mb-6'>
                <label htmlFor="githubUserName">GitHub Username</label>
                <input id="githubUserName" type="text" className='rounded-lg  border border-gray-400 h-12 p-3 bg-transparent' placeholder='' name='githubUserName' value={formData.githubUserName} onChange={handleChange}  />
            </div>
